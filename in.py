...
def hso_gmail(email):
   ...
   # print(response)
    if 'password' in response:
        info(email)
        GE+=1    
        sys.stdout.write(f'''\r{S}HSO • [{F}True : {M}{GE}{Z} | False: {M}{BE}{X} | Not: {M}{BI}{C} | {S}]'''),sys.stdout.flush()    
    else:
        BE+=1
        sys.stdout.write(f'''\r{S}HSO • [{F}True : {M}{GE}{Z} | False: {M}{BE}{X} | Not: {M}{BI}{C} | {S}]'''),sys.stdout.flush()    
...
def main():
     admin=f'''
     {F}<<{M}Instgram Toll Free Version  0.1{F}>>
{X}═════════════════════════════
{S}1 - {A}Random Gmail  
{S}2 - {A}Check List Gmail
{X}═════════════════════════════
{M}Devoleber : {X}[@ii33cc]
'''
     print(admin)
     try:
...
main()
#حقوق المطور حسو لا تتلاعب بل حقوق اذا حاولت تتلاعب بشي هذا يدل على فشلك يا فاشل 
